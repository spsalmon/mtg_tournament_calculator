...
describe('validateConfig', () => {
  ...
  it('rejects out-of-range and non-integer inputs', () => {
    // Two problems, not one: a one-player field also trips the cut-vs-field check,
    // and no cut value can isolate the first — any cut of at least 1 is also at
    // least the field size when the field is 1.
...
export default function CutField({ value, onChange }: CutFieldProps) {
  // Custom mode needs its own flag. Deriving it from `value` alone cannot work:
  // the cut starts on a preset and prefillFor only ever returns presets, so a
  // derived flag would never flip and the custom input would be unreachable.
...
The Custom button only flips local mode — it leaves `value` alone so the custom input opens pre-filled with the current cut. The mode flag has to be real state: deriving "is custom" from whether `value` matches a preset cannot work, because the cut starts on a preset and `prefillFor` only ever returns presets, so a derived flag would never flip and the custom input would never render.
